# -*- coding: utf-8 -*-
"""
Created on Wed Jun 19 11:36:26 2019
...
N次就能把所有的节点都取完了，如果N次操作结束还没判断出来，那么就不是DAG.在这N次中，每次都找一个入度为0的点，
并把它的入度变为-1，作为已经取过的点不再使用，同时把从这个点指向的点的入度都-1.

这个过程中，如果找不到入度为0的点，那么说明存在环。如果N次操作，每次都操作成功的去除了一个入度为0的点，
那么说明这个图是DAG.

时间复杂度是O(N ^ 2)，空间复杂度是O(N)。 BFS
--------------------- 
...
class Solution(object):
    def canFinish(self, N, prerequisites):
        ...
        for u, v in prerequisites:
            ...
            indegrees[u] += 1#
        for i in range(N):#这里其实应该维护一个队列 比如写成while queue之类的，但写这个也可以
            zeroDegree = False
            for j in range(N):
                if indegrees[j] == 0:
                    zeroDegree = True
            ...
            indegrees[j] = -1#在这N次中，每次都找一个入度为0的点，并把它的入度变为-1，
            for node in graph[j]:
                indegrees[node] -= 1#作为已经取过的点不再使用，同时把从这个点指向的点的入度都-1.
        return True                
...
class Solution1(object):
    ...
    def dfs(self, graph, visited, i):
        if visited[i] == 1: return False#如果判断过程中找到了当前的正在判断的路径，说明有环
        if visited[i] == 2: return True
        visited[i] = 1
        for j in graph[i]:
            if not self.dfs(graph, visited, j):#最后如果到达了无路可走的状态，说明当前节点是安全的。
                return False
        visited[i] = 2
        return True
...
if __name__ == "__main__":
    course = [[1,0],[2,1]]
    print(Solution().canFinish(3, course))