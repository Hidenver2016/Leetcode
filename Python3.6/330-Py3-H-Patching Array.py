# -*- coding: utf-8 -*-
"""
Created on Tue Feb  5 14:22:24 2019
...
所以我们要在数组里加上一个8，此时能表示的范围是[0, 16)，那么我们需要插入16吗，答案是不需要，
因为我们数组有1和4，可以组成5，而下一个数字11，加一起能组成16，所以有了数组中的11，
我们此时能表示的范围扩大到[0, 27)，但我们没法表示27，因为30太大了，所以此时我们给数组中加入一个27，
那么现在能表示的范围是[0, 54)，已经满足要求了，我们总共添加了两个数8和27，所以返回2即可。
"""
# Time:  O(s + logn), s is the number of elements in the array
# Space: O(1)

class Solution(object):#类似于2的幂和可以拼出任意自然数
    def minPatches(self, nums, n):
        ...
        while miss <= n:
            if i < len(nums) and nums[i] <= miss:
                ...
                miss += miss#过了就加自己即可,跳到gap的地方就加上自己比如上面的8，27
                patch += 1
