# -*- coding: utf-8 -*-
"""
Created on Sun Dec  2 15:00:42 2018
...
Go to the next heater if the current house coordinate 
is larger than or equal to the middle between the current and the next heater.

Time: O(nlogn(如果需要快速排序) + n(只需要遍历一次))
Space: O(1)
"""
class Solution:
    def findRadius(self, houses, heaters):
        ...
        for x in sorted(houses):
            while x >= sum(heaters[i:i+2]) / 2.: # 房子的位置大于两个邻近heater的中间值
                i += 1 #去新的heater
            r = max(r, abs(heaters[i] - x)) #heater位置的更新
        return r
