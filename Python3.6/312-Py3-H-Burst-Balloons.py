...
重点就是这个状态转移方程： 
dp[i][j]表示i到j的maxCoins是多少，自然答案就是dp[1][n] (因为左右都补了一个1)
...
class Solution1(object):
    def maxCoins(self, nums):
        ...
        for len_ in range(1, n + 1):#这个地方表示气球序列的长度[1, n]
            for left in range(1, n - len_ + 2):#左边left 是从[1, n-len+1],最左边留一个，实际上是（n+1-len+1）
        ...
        return dp[1][n] #关于上面的公式，可以反推，因为dp[1][n]是答案，就是计算了0和n+1的气球，所以dp[left][right]也是要计算[left-1]和[right+1]的
