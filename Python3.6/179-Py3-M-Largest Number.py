...
class Solution1:#看这个
    ...
    # @return a string
    def largestNumber(self, num):#sorted本身就是快排，所以里面的数字全都是要比较的
        num = sorted([str(x) for x in num], key = cmp_to_key(self.compare))#这里注意后面的数是b,前面的是a,所以compare中写成[1,-1],
        ans = ''.join(num).lstrip('0')#删除左边的"0"
        return ans or '0'

    def compare(self, a, b):#这里注意后面的数是b,前面的是a,所以compare中写成[1,-1],表示符合条件的逆序，不符合的顺序
        return [1, -1][a + b > b + a]#a + b > b + a时，逆序； 否则顺序


if __name__ == "__main__":
    print(Solution1().largestNumber([2,1,9,11,27,123]))
