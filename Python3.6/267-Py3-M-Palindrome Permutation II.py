...
class Solution:
    def generatePalindromes(self, s):
        ...
        half = [c for c in half]# 一半字符的list
        
        def backtrack(end, tmp):#一半字符的长度end, 临时的排列组合tmp   整个这个backtrack应该就是在做去掉重复字母的permutation
            if len(tmp) == end:#表示完成了一个排列组合，用一半的字母
                cur = ''.join(tmp)
                ...
                for i in range(end):
                    if visited[i] or (i>0 and half[i] == half[i-1] and not visited[i-1]):#后面这一半是用来处理重复字母的
                        continue
                    visited[i] = True
                    tmp.append(half[i])#加入tmp, 每次加入一个字母，就会把这个字母为首的所有排列过一遍，然后再置为False,弹出
                    backtrack(end, tmp)
...
if __name__ == "__main__":
    print(Solution().generatePalindromes("aabbbbccc"))
